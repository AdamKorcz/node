...
std::string S2 =
	"fs.writeFileSync('/tmp/test-fuzz', '";
//RANDOM
std::string S3 =
	"');\n";

std::string S4 =
	"fs.readFile('/tmp/test-fuzz', 'utf8', (err, data) => {\n"
	"  if (err) {\n"
	"    var _ = err\n"
	"    return;\n"
	"  }\n"
...
std::string S7 =
	"fs.appendFile('/tmp/test-fuzz', textToAppend,\n"
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data2, size_t size) {
  ...
  std::string s2 = prov.ConsumeRemainingBytesAsString();
